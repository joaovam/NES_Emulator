...
pub fn trace(cpu: &mut CPU) -> String{
    ...
    let tmp = match ops.len{
        ...
        2 => {
            ...
            match ops.mode {
                ...
                AddressingMode::ZeroPage => format!("${:02x} = {:02x}", addr, stored_value),

                AddressingMode::ZeroPage_X => format!("${:02x},X @ {:02x} = {:02x}", 
        ...
        3 => {
            ...
            let addr_lo = cpu.mem_read(pc + 1);
            let addr_hi = cpu.mem_read(pc + 2);
            hex.push(addr_lo);
            hex.push(addr_hi);
    ...
    let asm_str = format!("{:04x}  {:8} {: >4} {}", pc, hex_str, ops.mnemonic, tmp)
    .trim()
    .to_string();
...
mod test {
   ...
   fn test_format_mem_access() {
       let mut bus = Bus::new(test_rom(), |_: &crate::ppu::NesPPU,_| {});
       // ORA ($33), Y
